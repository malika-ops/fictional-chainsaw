...
public class GetFiltredSectorsQueryHandler
    : IQueryHandler<GetFiltredSectorsQuery, PagedResult<SectorResponse>>
{
    private readonly ISectorRepository _repo;

    public GetFiltredSectorsQueryHandler(ISectorRepository repo) => _repo = repo;

    public async Task<PagedResult<SectorResponse>> Handle(
        GetFiltredSectorsQuery sectorQuery, CancellationToken ct)
    {
