...
public class PatchPartnerCommandHandler : ICommandHandler<PatchPartnerCommand, Result<bool>>
{
    ...
    public async Task<Result<bool>> Handle(PatchPartnerCommand cmd, CancellationToken ct)
    {
        ...
        if (cmd.Code != null && cmd.Code != partner.Code)
        {
        ...
        // Check if tax identification number is unique (if being updated)
        if (cmd.TaxIdentificationNumber != null && cmd.TaxIdentificationNumber != partner.TaxIdentificationNumber)
        {
        ...
        // Check if ICE is unique (if being updated)
        if (cmd.ICE != null && cmd.ICE != partner.ICE)
        {
