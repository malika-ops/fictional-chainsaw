...
public class UpdateControleEndpointTests(TestWebApplicationFactory factory) : BaseAcceptanceTests(factory)
{
    ...
    public async Task Put_ShouldReturn409_WhenDuplicateCode()
    {
        ...
        _controleRepoMock.Setup(r => r.GetByIdAsync(ControleId.Of(id1), It.IsAny<CancellationToken>()))
             .ReturnsAsync(target);
        _controleRepoMock.Setup(r => r.GetOneByConditionAsync(
                        It.IsAny<System.Linq.Expressions.Expression<Func<Controle, bool>>>(),
                        It.IsAny<CancellationToken>()))
